...
def collect_kite_aerodynamics_outputs(options, atmos, wind, ua, ua_norm, q_eff, aero_coefficients, f_aero, f_lift, f_drag, f_side, m_aero, ehat_chord, ehat_span, r, q, n, outputs, parameters):
    ...
    outputs['aerodynamics']['air_velocity' + str(n)] = ua
    ...
    outputs['aerodynamics']['f_aero_earth' + str(n)] = f_aero
    ...
    f_aero_wind = frames.from_earth_to_wind(ua, kite_dcm, f_aero_earth)
    f_lift_earth = f_lift
    f_drag_earth = f_drag
    f_side_earth = f_side
    ...
    outputs['aerodynamics']['f_lift_earth' + str(n)] = f_lift
    outputs['aerodynamics']['f_drag_earth' + str(n)] = f_drag
    outputs['aerodynamics']['f_side_earth' + str(n)] = f_side
    ...
    if int(options['kite_dof']) == 6:
        outputs['aerodynamics']['m_aero_body' + str(n)] = m_aero
...
def collect_power_balance_outputs(variables, n, outputs, architecture):
    ...
    outputs['power_balance']['P_lift'+str(n)] = cas.mtimes(outputs['aerodynamics']['f_lift_earth'+str(n)].T, dq_n)
    outputs['power_balance']['P_drag'+str(n)] = cas.mtimes(outputs['aerodynamics']['f_drag_earth'+str(n)].T, dq_n)
    if 'r'+str(n)+str(architecture.parent_map[n]) in list(variables['xd'].keys()):
        outputs['power_balance']['P_side'+str(n)] = cas.mtimes(outputs['aerodynamics']['f_side_earth'+str(n)].T, dq_n)
