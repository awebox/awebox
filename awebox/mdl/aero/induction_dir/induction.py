...
def get_induction_cstr(options, atmos, wind, variables_si, parameters, outputs, architecture, objects):

    cstr_list = cstr_op.ConstraintList()

    general_trivial = get_general_trivial_residual(options, wind, variables_si, architecture, objects)
    general_final = get_general_final_residual(options, wind, variables_si, parameters, outputs, architecture, objects)
    ...
    specific_cstr = get_specific_cstr(options, atmos, wind, variables_si, parameters, objects, outputs, architecture)
    cstr_list.append(specific_cstr)

    return cstr_list


def get_general_trivial_residual(options, wind, variables_si, architecture, objects):
    resi = []
    ...
    any_vor = any(label[:3] == 'vor' for label in comparison_labels)
    if any_vor:
        vortex_resi = vortex.get_induction_trivial_residual(options, wind, variables_si, architecture, objects)
        resi = cas.vertcat(resi, vortex_resi)

    return resi

def get_general_final_residual(options, wind, variables_si, parameters, outputs, architecture, objects):
    resi = []
    ...
    any_vor = any(label[:3] == 'vor' for label in comparison_labels)
    if any_vor:
        vortex_resi = vortex.get_induction_final_residual(options, wind, variables_si, outputs, architecture, objects)
        resi = cas.vertcat(resi, vortex_resi)

    return resi


def get_specific_cstr(options, atmos, wind, variables_si, parameters, objects, outputs, architecture):
...
def collect_outputs(options, atmos, wind, variables_si, outputs, vortex_objects, parameters, architecture):
    ...
    any_vor = any(label[:3] == 'vor' for label in comparison_labels)
    if any_vor:
        outputs = vortex.collect_vortex_outputs(options, atmos, wind, variables_si, outputs, vortex_objects, parameters, architecture)
