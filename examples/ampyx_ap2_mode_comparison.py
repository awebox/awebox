...
def common_options():
    ...
    # options['model.system_bounds.x.l_t'] = [1.0e-2, 120.0]
    ...
    options['nlp.n_k'] = 40  # refine this value for longer periods
...
def drag_mode_options(options):
    ...
    options['model.system_bounds.theta.t_f'] = [4, 15]  # more than 25 seconds allows for ill solutions
    options['nlp.n_k'] = 40
    return options

def lift_mode_options(options):
    options['user_options.trajectory.system_type'] = 'lift_mode'
    options['user_options.trajectory.lift_mode.windings'] = 4
    options['model.system_bounds.theta.t_f'] = [20, 80]  # 80 seconds: 2 pumps
    options['nlp.n_k'] = 80

    return options

def base_rocking_mode_options(options):
...
def rocking_mode_example_0(options):
    options = base_rocking_mode_options(options)
    return options
...
def reduced_airspace(options, halfsize, zmin):
    mode = options['user_options.trajectory.system_type']
    if mode == 'lift_mode':
        options['model.system_bounds.theta.t_f'] = [5, 15]  # more than 25 seconds allows for ill solutions
        options['user_options.trajectory.lift_mode.windings'] = 2
    elif mode == 'drag_mode':
        options['model.system_bounds.theta.t_f'] = [2, 5]
    elif mode == 'rocking_mode':
        options['model.system_bounds.theta.t_f'] = [2, 5]
    
    options['solver.initialization.l_t'] = 50.0
    options['model.system_bounds.x.l_t'] = [5.0, 350.0]  # Halved
    options['model.system_bounds.x.dl_t'] = [-30.0, 40.0]  # Doubled
    options['model.system_bounds.x.ddl_t'] = [-4.8, 4.8]  # Doubled

    options['model.system_bounds.x.q'] = np.array([-np.inf, -halfsize, zmin]), np.array([halfsize, halfsize, halfsize])
    return options

def alter_arm_length(epsilon, options):
    options['user_options.trajectory.fixed_params']['arm_length'] += epsilon
    return options

def rocking_mode_options(options):
    return rocking_mode_example_2(options)

def free_arm_rocking_mode_options(options):
    options = rocking_mode_options(options)
    options['solver.initialization.theta.arm_length'] = None
    options['model.system_bounds.theta.arm_length'] = [1e-1, 1e3]
    return options
...
def get_step_results(trial, step_name):
    ...
    res['mean(wind^3)^(1/3) (m/s)'] = np.cbrt(np.mean(u_wind**3))
    res['Median wind speed (m/s)'] = np.median(u_wind)
    res['Median kite speed (m/s)'] = np.median(u_kite)
    res['q residual (m)'] = np.max(np.abs(plot_dict['outputs']['invariants']['c10']))
    res['dq residual (m/s)'] = np.max(np.abs(plot_dict['outputs']['invariants']['dc10']))
    res['ddq residual(m/s^2)'] = np.max(np.abs(plot_dict['outputs']['invariants']['ddc10']))
    
    return res

def run_task(subfolder, prefix, config_func, base_options, base_path):
    shutil.rmtree('cache', ignore_errors=True)
    
    header = f' STARTING TASK: {prefix} '
    print('\n' + '='*80)
    ...
    seed_path = os.path.join(save_dir, f'{prefix}_seed.pickle')
    options_path = os.path.join(save_dir, f'{prefix}_options.pickle')
    ...
    try:
        ...
        with open(seed_path, 'wb') as f:
            pickle.dump(trial.options_seed, f)

        with open(options_path, 'wb') as f:
            pickle.dump(trial.options, f)
        ...
        for step in steps:
            ...
            trial.plot(['states', 'quad', 'invariants'])
...
def main():
    ...
    # Define tasks: (Subfolder, Filename_Prefix, Function)
    tasks = [
        ('exp1', 'lift', lambda opt: lift_mode_options(opt)),
        ('exp1', 'drag', lambda opt: drag_mode_options(opt)),
        ('exp1', 'rocking', lambda opt: rocking_mode_example_2(opt)),
        # ('exp1', 'rocking_opt_arm', lambda opt: free_arm_rocking_mode_options(opt)),
        # ('exp2_diff-div', 'm1', lambda opt: alter_arm_length(-1e-1, rocking_mode_example_2(opt))),  # a - 1e-1
        # ('exp2_diff-div', 'm2', lambda opt: alter_arm_length(-1e-2, rocking_mode_example_2(opt))),  # a - 1e-2
        # ('exp2_diff-div', 'm3', lambda opt: alter_arm_length(-1e-3, rocking_mode_example_2(opt))),  # a - 1e-3
        # ('exp2_diff-div', 'p1', lambda opt: alter_arm_length(+1e-1, rocking_mode_example_2(opt))),  # a + 1e-1
        # ('exp2_diff-div', 'p2', lambda opt: alter_arm_length(+1e-2, rocking_mode_example_2(opt))),  # a + 1e-2
        # ('exp2_diff-div', 'p3', lambda opt: alter_arm_length(+1e-3, rocking_mode_example_2(opt))),  # a + 1e-3
        ('exp3_reduced_airspace', 'lift', lambda opt: reduced_airspace(lift_mode_options(opt), halfsize=50, zmin=5)),
        ('exp3_reduced_airspace', 'drag', lambda opt: reduced_airspace(drag_mode_options(opt), halfsize=50, zmin=5)),
        ('exp3_reduced_airspace', 'rocking', lambda opt: reduced_airspace(rocking_mode_example_2(opt), halfsize=50, zmin=5)),
        # ('exp3_reduced_airspace', 'rocking_opt_arm', lambda opt: reduced_airspace(free_arm_rocking_mode_options(opt), halfsize=50, zmin=5)),
    ]
    
    base_options = common_options()
    
    if MULTIPROCESSING:
        full_args = [t + (base_options, base_path) for t in tasks]
        run_task_wrapper = lambda args: run_task(*args, base_options, base_path)
        with Pool() as pool:
            pool.map(run_task_wrapper, tasks)
    else:
        for subfolder, prefix, func in tasks:
            run_task(subfolder, prefix, func, base_options, base_path)


def print_pickled_params(path):
...
if __name__ == '__main__':
    t0 = time.time()
    if MULTIPROCESSING and platform.system() == "Darwin":
        ctx._force_start_method('spawn')
    main()
    print("Total time: ", time.time() - t0)

# Modèles comparables car rendements 100%
# Romain: cycle limite avec le kite qui pousse le bras
# Modifier la statistique vent moyen pour intégrer puissance au cube et avoir une puissance disponible moyenne
# augmenter longueur du bras jusqu'à avoir une puissance disponible moyenne du même ordre que les autres
# Contrôle de ddα directement